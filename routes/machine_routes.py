...
async def get_all_machines():
    return await service.list_all_machines()

@router.put("/update-machine/{machine_name}")
async def update_machine(data:MachineBase, machine_name:str):
    return await service.update_machine(data, machine_name)

@router.delete("/delete-machine/{machine_name}")
async def delete_machine(machine_name):
    return await delete_machine(machine_name)

@router.post("bulk-upload")
async def bulk_upload(csv_file:UploadFile=File(...),files:list[UploadFile]=File(...)):
    return await service.bulk_import_machines(csv_file,files)